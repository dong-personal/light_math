...
struct product_type
{
  ...
  enum
  {
    ...
    MaxDepth =
        EIGEN_SIZE_MIN_PREFER_FIXED(traits<_Lhs>::MaxColsAtCompileTime, traits<_Rhs>::MaxRowsAtCompileTime),
...
struct gemv_static_vector_if<Scalar, Size, MaxSize, true>
{
  ...
  // => let's manually enforce alignment by allocating more data and return the address of the first aligned
  // element.
  internal::plain_array<
      Scalar, EIGEN_SIZE_MIN_PREFER_FIXED(Size, MaxSize) + (ForceAlignment ? EIGEN_MAX_ALIGN_BYTES : 0), 0>
      m_data;
  EIGEN_STRONG_INLINE Scalar* data()
  {
    return ForceAlignment ? reinterpret_cast<Scalar*>((internal::UIntPtr(m_data.array) &
...
struct gemv_dense_selector<OnTheLeft, StorageOrder, BlasCompatible>
{
  ...
  static void run(const Lhs& lhs, const Rhs& rhs, Dest& dest, const typename Dest::Scalar& alpha)
  {
    ...
    gemv_dense_selector<OnTheRight, OtherStorageOrder, BlasCompatible>::run(rhs.transpose(), lhs.transpose(),
                                                                            destT, alpha);
  }
};
...
struct gemv_dense_selector<OnTheRight, RowMajor, true>
{
  ...
  static void run(const Lhs& lhs, const Rhs& rhs, Dest& dest, const typename Dest::Scalar& alpha)
  {
    ...
    general_matrix_vector_product<Index, LhsScalar, LhsMapper, RowMajor, LhsBlasTraits::NeedToConjugate,
                                  RhsScalar, RhsMapper, RhsBlasTraits::NeedToConjugate>::
...
struct gemv_dense_selector<OnTheRight, ColMajor, false>
{
  ...
  static void run(const Lhs& lhs, const Rhs& rhs, Dest& dest, const typename Dest::Scalar& alpha)
  {
    ...
    // TODO if rhs is large enough it might be beneficial to make sure that dest is sequentially stored in

...
{
  EIGEN_EMPTY_STRUCT_CTOR(scalar_random_op) inline const Scalar operator()() const
  {
...
// template <typename Scalar>
// struct scalar_random_op<std::complex<Scalar> >
// {
//   mutable std::random_device rd;  // 用于初始化生成器
//   mutable std::mt19937 gen;       // 使用 Mersenne Twister 算法的生成器
//   mutable std::uniform_real_distribution<> dis;

//   scalar_random_op() : gen(rd()), dis(-1.0, 1.0) { ; }
//   inline const std::complex<Scalar> operator()() { return std::complex<Scalar>(dis(gen), dis(gen)); }
// };
 ...
 * \not_reentrant
 *
 * \sa DenseBase::setRandom(), setRandom(Index), setRandom(Index, NoChange_t), class CwiseNullaryOp,
 * DenseBase::Random()
 */
template <typename Derived>
 ...
 * \not_reentrant
 *
 * \sa DenseBase::setRandom(), setRandom(Index), setRandom(NoChange_t, Index), class CwiseNullaryOp,
 * DenseBase::Random()
 */
template <typename Derived>

...
// GCC 4.7 is too aggressive in its optimizations and remove the alignment test based on the fact the array is
// declared to be aligned. See this bug report: http://gcc.gnu.org/bugzilla/show_bug.cgi?id=53900 Hiding the
// origin of the array pointer behind a function argument seems to do the trick even if the function is
// inlined:
...
#define EIGEN_MAKE_UNALIGNED_ARRAY_ASSERT(sizemask)                                                      \
  eigen_assert((internal::UIntPtr(eigen_unaligned_array_assert_workaround_gcc47(array)) & (sizemask)) == \
                   0 &&                                                                                  \
...
struct plain_array_helper
{
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE static void swap(
      plain_array<T, Size, MatrixOrArrayOptions, Alignment>& a, const Eigen::Index a_size,
...
public:
  ...
  EIGEN_DEVICE_FUNC DenseStorage(const DenseStorage& other)
      : m_data(internal::conditional_aligned_new_auto<T, (_Options & DontAlign) == 0>(other.m_rows *
                                                                                      other.m_cols)),
        m_rows(other.m_rows),
        m_cols(other.m_cols)
  {

...
struct evaluator_assume_aliasing<
    CwiseBinaryOp<internal::scalar_product_op<Scalar1, Scalar2>,
...
// Dense += Product
template <typename DstXprType, typename Lhs, typename Rhs, int Options, typename Scalar>
struct Assignment<DstXprType, Product<Lhs, Rhs, Options>, internal::add_assign_op<Scalar, Scalar>,
                  Dense2Dense,
                  typename enable_if<(Options == DefaultProduct || Options == AliasFreeProduct)>::type>
{
  typedef Product<Lhs, Rhs, Options> SrcXprType;
...
// Dense -= Product
template <typename DstXprType, typename Lhs, typename Rhs, int Options, typename Scalar>
struct Assignment<DstXprType, Product<Lhs, Rhs, Options>, internal::sub_assign_op<Scalar, Scalar>,
                  Dense2Dense,
                  typename enable_if<(Options == DefaultProduct || Options == AliasFreeProduct)>::type>
{
  typedef Product<Lhs, Rhs, Options> SrcXprType;
...
template <typename DstXprType, typename Lhs, typename Rhs, typename AssignFunc, typename Scalar,
          typename ScalarBis, typename Plain>
...
struct evaluator_assume_aliasing<
    CwiseBinaryOp<internal::scalar_sum_op<typename OtherXpr::Scalar,
                                ...
                                generic_product_impl<Lhs, Rhs, DenseShape, DenseShape, GemvProduct> >
{
                                                                  ...
                                                                  const Scalar& alpha)
  {
    ...
    internal::gemv_dense_selector<
        ...
        bool(internal::blas_traits<MatrixType>::HasUsableDirectAccess)>::run(actual_lhs, actual_rhs, dst,
                                                                             alpha);
  }
};
...
struct generic_product_impl<Lhs, Rhs, DenseShape, DenseShape, CoeffBasedProductMode>
{
  ...
  // This is a special evaluation path called from generic_product_impl<...,GemmProduct> in file
  // GeneralMatrixMatrix.h This variant tries to extract scalar multiples from both the LHS and RHS and factor
  ...
  // For small fixed sizes matrices, howver, the gains are less obvious, it is sometimes x2 faster, but
  // sometimes x3 slower, and the behavior depends also a lot on the compiler... This is why this re-writting
  // strategy is currently enabled only when falling back from the main GEMM.
...
protected:
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void eval_dynamic_impl(Dst& dst, const LhsT& lhs,
    ...
    : evaluator_base<Product<Lhs, Rhs, LazyProduct> >
{
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE explicit product_evaluator(const XprType& xpr)
      : m_lhs(xpr.lhs()),
        m_rhs(xpr.rhs()),
        m_lhsImpl(
            m_lhs),  // FIXME the creation of the evaluator objects should result in a no-op, but check that!
        m_rhsImpl(m_rhs),  //       Moreover, they are only useful for the packet path, so we could completely
                           //       disable them when not needed, or perhaps declare them on the fly on the
                           //       packet method... We have experiment to check what's best.
  ...
  enum
  {
    ...
    Alignment = bool(CanVectorizeLhs)
                    ? (LhsOuterStrideBytes <= 0 ||
                               (int(LhsOuterStrideBytes) % EIGEN_PLAIN_ENUM_MAX(1, LhsAlignment)) != 0
                           ? 0
                           : LhsAlignment)
                : bool(CanVectorizeRhs)
                    ? (RhsOuterStrideBytes <= 0 ||
                               (int(RhsOuterStrideBytes) % EIGEN_PLAIN_ENUM_MAX(1, RhsAlignment)) != 0
                           ? 0
                           : RhsAlignment)
                    : 0,

    /* CanVectorizeInner deserves special explanation. It does not affect the product flags. It is not used
     * outside of Product. If the Product itself is not a packet-access expression, there is still a chance
     * that the inner loop of the product might be vectorized. This is the meaning of CanVectorizeInner. Since
     * it doesn't affect the Flags, it is safe to make this value depend on ActualPacketAccessBit, that
                                ...
                                generic_product_impl<Lhs, Rhs, TriangularShape, DenseShape, ProductTag> >
{
  ...
  {
    triangular_product_impl<Lhs::Mode, true, typename Lhs::MatrixType, false, Rhs,
                                ...
                                generic_product_impl<Lhs, Rhs, DenseShape, TriangularShape, ProductTag> >
{
  ...
  static void scaleAndAddTo(Dest& dst, const Lhs& lhs, const Rhs& rhs, const Scalar& alpha)
  {
    triangular_product_impl<Rhs::Mode, false, Lhs, Lhs::RowsAtCompileTime == 1, typename Rhs::MatrixType,
                                ...
                                generic_product_impl<Lhs, Rhs, SelfAdjointShape, DenseShape, ProductTag> >
{
  ...
  {
    selfadjoint_product_impl<typename Lhs::MatrixType, Lhs::Mode, false, Rhs, 0,
                                ...
                                generic_product_impl<Lhs, Rhs, DenseShape, SelfAdjointShape, ProductTag> >
{
  ...
  static void scaleAndAddTo(Dest& dst, const Lhs& lhs, const Rhs& rhs, const Scalar& alpha)
  {
    selfadjoint_product_impl<Lhs, 0, Lhs::IsVectorAtCompileTime, typename Rhs::MatrixType, Rhs::Mode,
                             false>::run(dst, lhs, rhs.nestedExpression(), alpha);
  }
};
...
{
  typedef
      typename ScalarBinaryOpTraits<typename MatrixType::Scalar, typename DiagonalType::Scalar>::ReturnType
          Scalar;
                                      ...
                                      OnTheLeft>
{
  ...
  {
    // FIXME: NVCC used to complain about the template keyword, but we have to check whether this is still the
    // case. See also similar calls below.
...
{
  template <typename Dest>
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void evalTo(Dest& dst, const Transpose<Lhs>& lhs,
                                                           const Rhs& rhs)
  {
...
{
  template <typename Dest>
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void evalTo(Dest& dst, const Lhs& lhs,

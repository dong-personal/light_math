...
struct traits<Reshaped<XprType, Rows, Cols, Order> > : traits<XprType>
{
  ...
  enum
  {
    ...
    // int(sizeof(Scalar))) % 16) == 0)) ? AlignedBit : 0,
          ...
          internal::traits<Reshaped<XprType, Rows, Cols, Order> >::HasDirectAccess>
{
  typedef internal::ReshapedImpl_dense<
      XprType, Rows, Cols, Order, internal::traits<Reshaped<XprType, Rows, Cols, Order> >::HasDirectAccess>
      Impl;
...
struct evaluator<Reshaped<ArgType, Rows, Cols, Order> >
    : reshaped_evaluator<ArgType, Rows, Cols, Order,
                        ...
                        typename Reshaped<ArgType, Rows, Cols, Order>::PlainObject>
{
  ...
  {
    // TODO: for the 3.4 release, this should be turned to an internal assertion, but let's keep it as is for
    // the beta lifetime
    eigen_assert(
        ((internal::UIntPtr(xpr.data()) % EIGEN_PLAIN_ENUM_MAX(1, evaluator<XprType>::Alignment)) == 0) &&
        "data is not aligned");
  }
};

...
struct packetwise_redux_impl<Func, Evaluator, CompleteUnrolling>
{
  ...
  EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE PacketType run(const Evaluator& eval, const Func& func,
                                                              Index /*size*/)
  {
    return redux_vec_unroller<Func, Evaluator, 0, packetwise_redux_traits<Func, Evaluator>::OuterSize>::
    ...
    : evaluator_base<PartialReduxExpr<ArgType, MemberOp, Direction> >
{
  ...
  {
    TraversalSize =
        Direction == int(Vertical) ? int(ArgType::RowsAtCompileTime) : int(ArgType::ColsAtCompileTime)
  };
  ...
  enum
  {
    ...
    _Vectorizable =
        bool(int(_ArgFlags) & PacketAccessBit) && bool(MemberOp::Vectorizable) &&
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC PacketType packet(Index idx) const
  {
    ...
    typedef Block<
        const ArgTypeNestedCleaned, Direction == Vertical ? int(ArgType::RowsAtCompileTime) : int(PacketSize),
    ...
    // FIXME
    // See bug 1612, currently if PacketSize==1 (i.e. complex<double> with 128bits registers) then the
    // storage-order of panel get reversed and methods like packetByOuterInner do not make sense anymore in
    // this context. So let's just by pass "vectorization" in this case:

 ...
 * element/row/columns of the underlying vector or matrix once passed to DenseBase::operator()(const
 * RowIndices&, const ColIndices&).
 *
...
// IndexedViewCompatibleType/makeIndexedViewCompatible turn an arbitrary object of type T into something
// usable by MatrixSlice The generic implementation is a no-op
...
// Turn a single index into something that looks like an array (i.e., that exposes a .size(), and
// operator[](int) methods)
template <typename T, int XprSize>
struct IndexedViewCompatibleType<T, XprSize,
...
template <typename T>
typename enable_if<symbolic::is_symbolic<T>::value, SingleRange>::type makeIndexedViewCompatible(
    const T& id, Index size, SpecializedType)
{
...
/** \var all
 * \ingroup Core_Module
 * Can be used as a parameter to DenseBase::operator()(const RowIndices&, const ColIndices&) to index all rows
 * or columns
 */

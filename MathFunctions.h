 ...
 * want to know is that it inherits ArrayBase. So we implement a partial specialization of sin_impl for
 * ArrayBase<Derived>. So we must make sure to use sin_impl<ArrayBase<Derived> > and not sin_impl<Derived>,
 * otherwise our partial specialization won't be used. How does sin know that? That's exactly what
 * global_math_functions_filtering_base tells it.
 *
 * How it's implemented:
 * SFINAE in the style of enable_if. Highly susceptible of breaking compilers. With GCC, it sure does work,
 * but if you replace the typename dummy by an integer template parameter, it doesn't work anymore!
 */
...
struct count_bits_impl
{
  ...
  {
    EIGEN_STATIC_ASSERT(is_integral<BitsType>::value && !NumTraits<BitsType>::IsSigned,
...
template <typename BitsType>
struct count_bits_impl<BitsType, typename enable_if<sizeof(unsigned int) < sizeof(BitsType) &&
                                                    sizeof(BitsType) <= sizeof(unsigned long)>::type>
{
  static const int kNumBits = static_cast<int>(sizeof(BitsType) * CHAR_BIT);
  static EIGEN_DEVICE_FUNC inline int clz(BitsType bits)
  {
...
template <typename BitsType>
struct count_bits_impl<BitsType, typename enable_if<sizeof(unsigned long) < sizeof(BitsType) &&
                                                    sizeof(BitsType) <= sizeof(__int64)>::type>
{
  static const int kNumBits = static_cast<int>(sizeof(BitsType) * CHAR_BIT);
  static EIGEN_DEVICE_FUNC inline int clz(BitsType bits)
  {
...
// std::is* do not work with fast-math and gcc, std::is* are available on MSVC 2013 and newer, as well as in
// clang.
...
template <typename T>
EIGEN_DEVICE_FUNC typename internal::enable_if<internal::is_integral<T>::value, bool>::type isnan_impl(
    const T&)
{
  return false;
}
...
template <typename T>
EIGEN_DEVICE_FUNC typename internal::enable_if<internal::is_integral<T>::value, bool>::type isinf_impl(
    const T&)
{
  return false;
}
...
template <typename T>
EIGEN_DEVICE_FUNC typename internal::enable_if<internal::is_integral<T>::value, bool>::type isfinite_impl(
    const T&)
{
  return true;
}
...
#else
// NOTE the inline qualifier and noinline attribute are both needed: the former is to avoid linking issue
// (duplicate symbol),
...
EIGEN_DEVICE_FUNC inline
    typename internal::add_const_on_value_type<EIGEN_MATHFUNC_RETVAL(real_ref, Scalar)>::type
...
template <typename Scalar>
EIGEN_DEVICE_FUNC inline
    typename internal::add_const_on_value_type<EIGEN_MATHFUNC_RETVAL(imag_ref, Scalar)>::type

 ...
 *
 * This class represents a triangular part of a matrix, not necessarily square. Strictly speaking, for
 * rectangular matrices one should speak of "trapezoid" parts. This class is the return type of
 * MatrixBase::triangularView() and SparseMatrixBase::triangularView(), and most of the time this is the only
 * way it is used.
 *
...
protected:
  ...
  typedef
      typename internal::remove_all<typename MatrixType::ConjugateReturnType>::type MatrixConjugateReturnType;
...
public:
  ...
  EIGEN_DEVICE_FUNC inline
      typename internal::conditional<Cond, ConjugateReturnType, ConstTriangularView>::type
      conjugateIf() const
  {
  ...
  typedef TriangularView<const typename MatrixType::ConstTransposeReturnType, TransposeMode>
...
// workaround MSVC ICE
  ...
  /** \returns a selfadjoint view of the referenced triangular part which must be either \c #Upper or \c
   * #Lower.
   *
 ...
 *
 * This class is an abstract base class of class TriangularView, and objects of type TriangularViewImpl cannot
 * be instantiated. It extends class TriangularView with additional methods which available for dense
 * expressions only.
 *
...
public:
   ...
   *
   * \warning The parameter is only marked 'const' to make the C++ compiler accept a temporary expression
   * here. This function will const_cast it, so constness isn't honored here.
   *
   * Note that the template parameter \c Side can be omitted, in which case \c Side==OnTheLeft
   *
...
//      such that Transpose<TriangularView<.,.> > is valid. (currently TriangularBase::transpose() is
 ...
 * The main difference is that the triangular, diagonal, and opposite parts are processed through three
 * different functions. \tparam UpLo must be either Lower or Upper \tparam Mode must be either 0, UnitDiag,
 * ZeroDiag, or SelfAdjoint
 */
template <int UpLo, int Mode, int SetOpposite, typename DstEvaluatorTypeT, typename SrcEvaluatorTypeT,
...
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void call_triangular_assignment_loop(DstXprType& dst,
                                                                           ...
                                                                           const Functor& func)
{
  ...
  triangular_assignment_loop<Kernel, Mode, unroll ? int(DstXprType::SizeAtCompileTime) : Dynamic,

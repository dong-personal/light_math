...
public:
  typedef
      typename find_best_packet<typename Evaluator::Scalar, Evaluator::SizeAtCompileTime>::type PacketType;
  enum
  {
...
// #define SCALAROP(x) scalar_##x##_op
// #define SYMBOL(x) SYMBOL_##x
// #define SYMBOL_sum +
// #define SYMBOL_sub -
// #define STRINGFY(x) #x

// #if (defined(EIGEN_HAS_OPENMP) && !defined(NO_OPENMP))
// #define REDUX_IMPL_OMP(x) _Pragma(STRINGFY(omp parallel for reduction(SYMBOL(x) : res)))

// #define REDUX_IMPL(x)                                                                            \
//   template <typename Lhs, typename Rhs, typename Evaluator>                                      \
//   struct redux_impl<SCALAROP(x) < Lhs, Rhs>, Evaluator, LinearVectorizedTraversal, NoUnrolling > \
//   {                                                                                              \
//     using Func = SCALAROP(x)<Lhs, Rhs>;                                                          \
//     typedef typename Evaluator::Scalar Scalar;                                                   \
//     template <typename XprType>                                                                  \
//     static Scalar run(const Evaluator& eval, const Func& func, const XprType& xpr)               \
//     {                                                                                            \
...
//       Scalar res;                                                                                \
//       REDUX_IMPL_OMP(x)                                                                          \
//       for (Index index = 0; index < size; ++index) res = func(res, eval.coeff(index));           \
//     }                                                                                            \
//   };
// REDUX_IMPL(sum)
// #endif
                                                          ...
                                                          const XprType& xpr)
  {
    ...
    if (VectorizedSize > 0)
    {
      ...
      if (VectorizedSize != Size)
        res = func(res, redux_novec_unroller<Func, Evaluator, VectorizedSize, Size - VectorizedSize>::run(
                            eval, func));
      return res;
    }
    else
...
public:
  ...
  enum
  {
    ...
    // TODO we should not remove DirectAccessBit and rather find an elegant way to query the alignment offset
    // at runtime from the evaluator
  ...
  {
    return Base::template packet<LoadMode, PacketType>(IsRowMajor ? outer : inner,
 ...
 */
template <typename Derived>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename internal::traits<Derived>::Scalar DenseBase<Derived>::sum()
    const
{
 ...
 */
template <typename Derived>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename internal::traits<Derived>::Scalar DenseBase<Derived>::mean()
    const
{
 ...
 */
template <typename Derived>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename internal::traits<Derived>::Scalar DenseBase<Derived>::prod()
    const
{

...
public:
  ...
  typedef PlainArray PlainObject;
   ...
   * are Matrix::resize() and Array::resize(). The present method only asserts that the new size equals the
   * old size, and does nothing else.
   */
  EIGEN_DEVICE_FUNC
  ...
  typedef
      typename internal::add_const_on_value_type<typename internal::eval<Derived>::type>::type EvalReturnType;
  /** \returns the matrix or vector obtained by evaluating this expression.
   *
   * Notice that in the case of a plain matrix or vector (not an expression) this function just returns
   * a const reference, in order to avoid a useless copy.
   *
   * \warning Be careful with eval() and the auto C++ keyword, as detailed in this \link
   * TopicPitfalls_auto_keyword page \endlink.
   */
  EIGEN_DEVICE_FUNC
  ...
  EIGEN_DEVICE_FUNC inline const typename internal::conditional<Enable, ForceAlignedAccess<Derived>,
                                                                Derived&>::type
  ...
  EIGEN_DEVICE_FUNC inline typename internal::traits<Derived>::Scalar minCoeff(IndexType* row,
                                                                               IndexType* col) const
  {
    return minCoeff<PropagateFast>(row, col);
  }
  template <typename IndexType>
  EIGEN_DEVICE_FUNC inline typename internal::traits<Derived>::Scalar maxCoeff(IndexType* row,
                                                                               IndexType* col) const
  {
  ...
  /** STL-like <a
   * href="https://en.cppreference.com/w/cpp/named_req/RandomAccessIterator">RandomAccessIterator</a> iterator
   * type as returned by the begin() and end() methods.
   */
...
#else
  typedef
      typename internal::conditional<(Flags & DirectAccessBit) == DirectAccessBit,
  ...
  typedef
      typename internal::conditional<IsVectorAtCompileTime, const_iterator_type, void>::type const_iterator;
#endif
...
protected:
  ...
  EIGEN_DEVICE_FUNC DenseBase()
  {
    ...
    EIGEN_STATIC_ASSERT(
        (EIGEN_IMPLIES(MaxRowsAtCompileTime == 1 && MaxColsAtCompileTime != 1, int(IsRowMajor)) &&

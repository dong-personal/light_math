...
{
  typedef
      typename conditional<is_arithmetic<T>::value, T, typename add_const_on_value_type<T>::type>::type type;
};
}  // namespace internal
...
public:
  ...
  // references to coeffs, which means exactly that we can have coeff() return a const reference (as opposed
  // to returning a value).
  // - The is_artihmetic check is required since "const int", "const double", etc. will cause warnings on some
  // systems while the declaration of "const T", where T is a non arithmetic type does not. Always returning
  // "const Scalar&" is not possible, since the underlying expressions might not offer a valid address the
  // reference could be referring to.
  typedef
      typename internal::conditional<bool(internal::traits<Derived>::Flags& LvalueBit), const Scalar&,
 ...
 * inherits DenseCoeffsBase<Derived, ReadOnlyAccessors> which defines functions to access entries read-only
 * using \c operator() .
 *
...
public:
  ...
  /** \returns the pointer increment between two consecutive inner slices (for example, between two
   * consecutive columns in a column-major matrix).
   *
 ...
 *
 * This class defines functions to work with strides which can be used to access entries directly. This class
 * inherits DenseCoeffsBase<Derived, WriteAccessors> which defines functions to access entries read/write
 * using \c operator().
 *
 * \sa \blank \ref TopicClassHierarchy
 */
...
/** \internal \returns the index of the first element of the array stored by \a m that is properly aligned
with respect to \a Alignment for vectorization.
  *

...
/// \internal EIGEN_COMP_MSVC_STRICT set to 1 if the compiler is really Microsoft Visual C++ and not ,e.g.,
/// ICC or clang-cl
...
/// \internal EIGEN_GNUC_STRICT set to 1 if the compiler is really GCC and not a compatible compiler (e.g.,
/// ICC, clang, mingw, etc.)
#if EIGEN_COMP_GNUC && !(EIGEN_COMP_CLANG || EIGEN_COMP_ICC || EIGEN_COMP_MINGW || EIGEN_COMP_PGI || \
...
#define EIGEN_HAS_STD_INVOKE_RESULT 0
    ...
    (__has_feature(cxx_alignas) || EIGEN_HAS_CXX14 || (EIGEN_COMP_MSVC >= 1800) ||           \
     (EIGEN_GNUC_AT_LEAST(4, 8)) || (EIGEN_COMP_CLANG >= 305) || (EIGEN_COMP_ICC >= 1500) || \
...
// ^^ Disable the use of variadic templates when compiling with versions of nvcc older than 8.0 on ARM
// devices:
...
#if defined(EIGEN_CUDACC)
// Const expressions are supported provided that c++11 is enabled and we're using either clang or nvcc 7.5 or
// above
...
// but it still doesn't use GCC's always_inline. This is useful in (common) situations where MSVC needs
// forceinline but GCC is still doing fine with just inline.
...
// EIGEN_ALWAYS_INLINE is the stronget, it has the effect of making the function inline and adding every
// possible attribute to maximize inlining. This should only be used when really necessary: in particular, it
// uses __attribute__((always_inline)) on GCC, which most of the time is useless and can severely harm compile
// times.
...
//           So static causes the resulting linked executable to be bloated with multiple copies of the same
//           function.
...
namespace internal
{
// trivial function copying a bool. Must be EIGEN_DONT_INLINE, so we implement it after including Eigen
// headers. see bug 89.
namespace
{
 ...
 * This is necessary, because the implicitly defined assignment operator gets deleted when a custom operator=
 * is defined. With C++11 or later this also default-implements the copy-constructor
 */
...
#define EIGEN_GENERIC_PUBLIC_INTERFACE(Derived)                                                              \
  ...
  typedef typename Eigen::NumTraits<Scalar>::Real                                                            \
      RealScalar; /*!< \brief The underlying numeric type for composed scalar types. \details In cases where \
                     Scalar is e.g. std::complex<T>, T were corresponding to RealScalar. */                  \
  typedef typename Base::CoeffReturnType                                                                     \
      CoeffReturnType; /*!< \brief The return type for coefficient access. \details Depending on whether the \
                          object allows direct coefficient access (e.g. for a MatrixXd), this type is either \
                          'const Scalar&' or simply 'Scalar' for objects that do not allow direct            \
                          coefficient access. */                                                             \
...
// EIGEN_SIZE_MIN_PREFER_DYNAMIC gives the min between compile-time sizes. 0 has absolute priority, followed
// by 1, followed by Dynamic, followed by other finite values. The reason for giving Dynamic the priority over
...
// difference is that finite values now have priority over Dynamic, so that min(3, Dynamic) gives 3. Indeed,
// whatever the actual value is (between 0 and 3), it is not more than 3.
...
#define EIGEN_SCALAR_BINARY_SUPPORTED(OPNAME, TYPEA, TYPEB)                                          \
  (Eigen::internal::has_ReturnType<Eigen::ScalarBinaryOpTraits<                                      \
       TYPEA, TYPEB, EIGEN_CAT(EIGEN_CAT(Eigen::internal::scalar_, OPNAME), _op) < TYPEA, TYPEB> > > \
   ::value)
...
#define EIGEN_MAKE_SCALAR_BINARY_OP_ONTHERIGHT(METHOD, OPNAME)                                              \
  ...
  EIGEN_MSVC10_WORKAROUND_BINARYOP_RETURN_TYPE(const EIGEN_EXPR_BINARYOP_SCALAR_RETURN_TYPE(                \
      Derived,                                                                                              \
  ...
  {                                                                                                         \
    typedef typename internal::promote_scalar_arg<                                                          \
        Scalar, T, EIGEN_SCALAR_BINARY_SUPPORTED(OPNAME, Scalar, T)>::type PromotedT;                       \
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE friend EIGEN_MSVC10_WORKAROUND_BINARYOP_RETURN_TYPE(            \
      const EIGEN_SCALAR_BINARYOP_EXPR_RETURN_TYPE(                                                     \
          typename internal::promote_scalar_arg<                                                        \
              Scalar EIGEN_COMMA T EIGEN_COMMA EIGEN_SCALAR_BINARY_SUPPORTED(OPNAME, T, Scalar)>::type, \
          Derived, OPNAME))(METHOD)(const T& scalar, const StorageBaseType& matrix)                     \
  {                                                                                                     \
    typedef typename internal::promote_scalar_arg<                                                      \
        Scalar, T, EIGEN_SCALAR_BINARY_SUPPORTED(OPNAME, T, Scalar)>::type PromotedT;                   \
...
#define EIGEN_MAKE_SCALAR_BINARY_OP(METHOD, OPNAME)     \
  ...
  EIGEN_MAKE_SCALAR_BINARY_OP_ONTHERIGHT(METHOD, OPNAME)
    ...
    !defined(EIGEN_EXCEPTIONS) && !defined(EIGEN_USE_SYCL) && !defined(EIGEN_HIP_DEVICE_COMPILE)
...
// The all function is used to enable a variadic version of eigen_assert which can take a parameter pack as
// its input.
namespace Eigen
{
namespace internal

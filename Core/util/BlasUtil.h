...
template <typename Scalar, typename Index, typename DataMapper, int Pack1, int Pack2, typename Packet,
...
public:
  ...
  // storePacketBlock_helper defines a way to access values inside the PacketBlock, this is essentially
  // required by the Complex types.
  ...
  // This function stores a PacketBlock on m_data, this approach is really quite slow compare to Incr=1 and
...
struct blas_traits
{
  ...
  typedef
      typename conditional<bool(HasUsableDirectAccess), ExtractType, typename _ExtractType::PlainObject>::type
    ...
    : blas_traits<NestedXpr>
{
  ...
  typedef blas_traits<NestedXpr> Base;
  typedef CwiseBinaryOp<scalar_product_op<Scalar>, const CwiseNullaryOp<scalar_constant_op<Scalar>, Plain>,
                        NestedXpr>
      XprType;
  typedef typename Base::ExtractType ExtractType;
...
template <typename Scalar, typename NestedXpr, typename Plain>
struct blas_traits<CwiseBinaryOp<scalar_product_op<Scalar>, NestedXpr,
...
struct combine_scalar_factors_impl
{
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE static ResScalar run(const ResScalar& alpha, const Lhs& lhs,
                                                             const Rhs& rhs)
  {

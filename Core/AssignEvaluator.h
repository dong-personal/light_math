...
private:
  enum
  {
    ...
    MayInnerVectorize = MightVectorize && int(InnerSize) != Dynamic &&
                        int(InnerSize) % int(InnerPacketSize) == 0 && int(OuterStride) != Dynamic &&
...
public:
  ...
  typedef typename conditional<int(Traversal) == LinearVectorizedTraversal, LinearPacketType,
  ...
  enum
  {
    Unrolling = (int(Traversal) == int(InnerVectorizedTraversal) || int(Traversal) == int(DefaultTraversal))
                ...
                : int(Traversal) == int(LinearVectorizedTraversal)
                    ? (bool(MayUnrollCompletely) && (EIGEN_UNALIGNED_VECTORIZE ||
...
struct dense_assignment_loop<Kernel, SliceVectorizedTraversal, NoUnrolling>
{
  EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE void run(Kernel &kernel)
  {
    ...
    const Index alignedStep =
        alignable ? (packetSize - kernel.outerStride() % packetSize) & packetAlignedMask : 0;
...
#pragma omp parallel for
    ...
    for (Index outer = 0; outer < kernel.outerSize(); ++outer)
    {
      ...
      copy_using_evaluator_DefaultTraversal_InnerUnrolling<Kernel, vectorizableSize, innerSize>::run(kernel,
                                                                                                     outer);
    }
  }
};
...
// Special kernel used when computing small products whose operands have dynamic dimensions.  It ensures that
// the PacketSize used is no larger than 4, thereby increasing the chance that vectorized instructions will be
// used when computing the product.
...
public:
  ...
  EIGEN_DEVICE_FUNC restricted_packet_dense_assignment_kernel(DstEvaluatorTypeT &dst,
...
// The only purpose of this call_assignment() function is to deal with noalias() / "assume-aliasing" and
// automatic transposition. Indeed, I (Gael) think that this concept of "assume-aliasing" was a mistake, and
// it makes thing quite complicated. So this intermediate function removes everything related to
// "assume-aliasing" such that Assignment does not has to bother about these annoying details.
...
template <typename Dst, typename Src, typename Func>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void call_restricted_packet_assignment_no_alias(Dst &dst,
                                                                                      const Src &src,
                                                                                      const Func &func)
{

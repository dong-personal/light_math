...
public:
   ...
   * TriangularView of the nested expression, otherwise, the nested expression is first transposed, thus
   * returning a \c TriangularView<Transpose<MatrixType>> object.
   *
      ...
      triangularView() const
  {
    ...
    return typename internal::conditional<
        (TriMode & (Upper | Lower)) == (UpLo & (Upper | Lower)), TriangularView<MatrixType, TriMode>,
  ...
  EIGEN_DEVICE_FUNC inline
      typename internal::conditional<Cond, ConjugateReturnType, ConstSelfAdjointView>::type
      conjugateIf() const
  {
   ...
   *
   * This method simply returns the diagonal of the nested expression, thus by-passing the SelfAdjointView
   * decorator.
   *
...
//      such that Transpose<SelfAdjointView<.,.> > is valid. (currently TriangularBase::transpose() is
...
template <int UpLo, int SetOpposite, typename DstEvaluatorTypeT, typename SrcEvaluatorTypeT, typename Functor,
          int Version>
class triangular_dense_assignment_kernel<UpLo, SelfAdjoint, SetOpposite, DstEvaluatorTypeT, SrcEvaluatorTypeT,
                                         Functor, Version>
...
/** \returns an expression of a symmetric/self-adjoint view extracted from the upper or lower triangular part

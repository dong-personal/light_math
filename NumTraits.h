 ...
 *     is a typedef to \a U.
 * \li A typedef \c NonInteger, giving the type that should be used for operations producing non-integral
 * values, such as quotients, square roots, etc. If \a T is a floating-point type, then this typedef just
 * gives \a T again. Note however that many Eigen functions such as internal::sqrt simply refuse to take
 * integers. Outside of a few cases, Eigen doesn't do automatic type promotion. Thus, this typedef is only
 * intended as a helper for code that needs to explicitly promote types. \li A typedef \c Literal giving the
 * type to use for numeric literals such as "2" or "0.5". For instance, for \c std::complex<U>, Literal is
 * defined as \c U. Of course, this type must be fully compatible with \a T. In doubt, just use \a T here. \li
 * A typedef \a Nested giving the type to use to nest a value inside of the expression tree. If you don't know
 * what this means, just use \a T here. \li An enum value \a IsComplex. It is equal to 1 if \a T is a \c
 * std::complex type, and to 0 otherwise. \li An enum value \a IsInteger. It is equal to \c 1 if \a T is an
 * integer type such as \c int, and to \c 0 otherwise. \li Enum values ReadCost, AddCost and MulCost
 * representing a rough estimate of the number of CPU cycles needed to by move / add / mul instructions
 * respectively, assuming the data is already stored in CPU registers. Stay vague here. No need to do
 * architecture-specific stuff. If you don't know what this means, just use \c Eigen::HugeCost. \li An enum
 * value \a IsSigned. It is equal to \c 1 if \a T is a signed type and to 0 if \a T is unsigned. \li An enum
 * value \a RequireInitialization. It is equal to \c 1 if the constructor of the numeric type \a T must be
 ...
 * href="http://en.cppreference.com/w/cpp/types/numeric_limits/epsilon">std::numeric_limits::epsilon()</a>, it
 * returns a \a Real instead of a \a T. \li A dummy_precision() function returning a weak epsilon value. It is
 * mainly used as a default value by the fuzzy comparison operators. \li highest() and lowest() functions
 * returning the highest and lowest possible values respectively. \li digits() function returning the number
 * of radix digits (non-sign digits for integers, mantissa for floating-point). This is the analogue of <a
 * href="http://en.cppreference.com/w/cpp/types/numeric_limits/digits">std::numeric_limits<T>::digits</a>
 *     which is used as the default implementation if specialized.
 * \li digits10() function returning the number of decimal digits that can be represented without change. This
 * is the analogue of <a
 * href="http://en.cppreference.com/w/cpp/types/numeric_limits/digits10">std::numeric_limits<T>::digits10</a>
 *     which is used as the default implementation if specialized.
 * \li min_exponent() and max_exponent() functions returning the highest and lowest possible values,
 * respectively, such that the radix raised to the power exponent-1 is a normalized floating-point number.
...
// Empty specialization for void to allow template specialization based on NumTraits<T>::Real with T==void and
// SFINAE.
template <>
